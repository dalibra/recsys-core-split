...
class SeqRecBase(pl.LightningModule):
    ...
    def filter_seen_items(self, preds, scores, seen_items):
        ...
        for i in range(preds.size(0)):
            ...
            pred = preds[i, not_seen_indexes]
            score = scores[i, not_seen_indexes]
            if pred.numel() < self.predict_top_k:
                pad_len = self.predict_top_k - pred.numel()
                pred = torch.cat(
                    [pred, pred.new_full((pad_len,), self.padding_idx)]
                )
                score = torch.cat(
                    [score, score.new_full((pad_len,), float("-inf"))]
                )
            else:
                pred = pred[:self.predict_top_k]
                score = score[:self.predict_top_k]
