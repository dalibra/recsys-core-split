...
except Exception:  # pragma: no cover - optional dependency / restricted env
    Task = None
...
def training(model, train_loader, eval_loader, config, task=None, retrain=False):
    ...
    result_tag = getattr(config, "result_tag", None)
    if result_tag:
        file_name = f"{file_name}__{result_tag}"
    ...
    try:
        ...
        if not getattr(trainer, "interrupted", False):
            if best_path and os.path.exists(best_path):
                seqrec_module.load_state_dict(torch.load(best_path)['state_dict'])
                os.rename(best_path, checkpoint_file)
                print(f"Checkpoint renamed to: {checkpoint_file}")
                num_best_epoches = torch.load(checkpoint_file)['epoch'] + 1
            else:
                print("No checkpoint produced; skipping checkpoint load/rename.")
                num_best_epoches = config.trainer_params.max_epochs
        else:
...
def evaluate(recs, ground_truth, task, config, num_items=None, prefix='test', successive=False):    
    if not successive:
        ...
        if config.get("save_local_per_user_metrics", False):
            per_user = compute_per_user_metrics(
                recs=recs,
                ground_truth=ground_truth,
                top_ks=list(config.evaluator.top_k),
            )
            save_local_per_user_metrics(per_user, config, prefix)
    else:
...
def compute_per_user_metrics(recs, ground_truth, top_ks):
    """Return compact per-user ranking contributions for paired bootstrap."""
    max_k = max(top_ks)
    gt = ground_truth[["user_id", "item_id"]].drop_duplicates("user_id")
    targets = dict(zip(gt["user_id"].tolist(), gt["item_id"].tolist()))

    rows = []
    for user_id, user_recs in recs.groupby("user_id", sort=False):
        target = targets.get(user_id)
        if target is None:
            continue
        ranked_items = user_recs["item_id"].to_numpy()[:max_k]
        hit_positions = np.flatnonzero(ranked_items == target)
        rank = int(hit_positions[0] + 1) if hit_positions.size else 0
        row = {
            "user_id": user_id,
...
def result_file_name(config):
    ...
    if config.model.model_class == 'SASRec':
        ...
        raise ValueError(f"Unsupported model class: {config.model.model_class}")
    result_tag = getattr(config, "result_tag", None)
    if result_tag:
        file_name = f"{file_name}__{result_tag}"
...
def local_results_path(config, prefix):
    split_subtype = config.split_subtype or ''
    q = 'q0'+str(config.quantile)[2:] if config.split_type == 'global_timesplit' else ''
...
def save_local_per_user_metrics(per_user, config, prefix):
    results_path = local_results_path(config, prefix + "_per_user")
    per_user.to_csv(os.path.join(results_path, result_file_name(config)), index=False)


def save_local_metrics(metrics, config, prefix):
    
    results_path = local_results_path(config, prefix)
    file_name = result_file_name(config)
